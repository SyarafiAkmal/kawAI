...
class NaiveBayes(Model):
    ...
    def predict(self, query):
        ...
        return sorted(result, key=lambda x: x[1], reverse=True)[0][0] # returns the class with the highest probability value
    ...
    def score(self, params = None):
        ...
        for i in range(len(self.data_test_x)):
            pred = self.predict(self.data_test_x[i])
            if pred == self.data_test_y[i][0]:
                tp += 1
...
if __name__ == "__main__":
    ...
    naiveB = NaiveBayes(data_csv, 0.5)
    end_time = time.time()
    print(naiveB.score(), "time:", end_time - start_time)
